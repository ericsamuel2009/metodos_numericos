<template>
  <div id="pagosEfectuados">
      ...
      <button @click="agregarPagoEfectuado">Nuevo Egreso</button><br>
          ...
          <input type="date" placeholder="fecha pago" v-model="fechaPago" @change="cambioFecha">
      ...
      <table class="tabla">
          <strong v-if="!datosEfectuados || datosEfectuados.length == 0">no hay datos</strong>
          <small v-if="carga">Cargando</small>
          <tr>
              <th>Grupo</th> <!-- SI ES VASS U OTRA ENTIDAD -->
              <th>Concepto</th> <!--SI FUE PRIMA O NOMINA U OTRA COSA-->
              <th>Monto</th> <!-- CANTIDAD DE DINERO-->
              <th>está Pagado?</th> <!-- COMENTARIO ADICIONAL -->
              <th>Metodo Pago</th> <!-- COMENTARIO ADICIONAL -->
              <th>Mes Pago</th> <!-- FECHA DE PAGO -->
              <th>Comentario</th> <!-- COMENTARIO ADICIONAL -->
          ...
          <tr v-for="main in datosEfectuados" :key="main.referencia">
              ...
              <td>
                <select name="" id="" v-model="main.estadoPago" disabled>
                    <option value="0">NO</option>
                    <option value="1">SI</option>
                </select>
              </td>
              <td>
                <select name="" id="" v-model="main.metodoPago" disabled>
                    <option value="EFECTIVO">EFECTIVO</option>
                    <option value="CUENTA">CUENTA</option>
                </select>
              </td>
              <td>
                <select name="" id="" :value="soloMes" disabled>
